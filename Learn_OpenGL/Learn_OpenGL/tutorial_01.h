...
bool create_window() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// check the call function and exchange buffer
		glfwPollEvents();
...
bool draw_triangle() {
	...
	// GLAD: initial and load all openGL function pointers
	if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress)) {
		cout << "Failed to initialize GLAD" << endl;
		return false;
	}
	...
	if (!success) {
		...
		cout << "ERROR::SHADER::LINK_SHADER_PROGRAM_FAILED\n" << infoLog << endl;
		return false;
	}
...
bool exe2_drawTwoTriWithDifferentVAOAndVBO() {
	// GLFW: initialize and configure
	glfwInit();
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
	glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);
...
bool exe3_drawTriangleWithTwoFragmentShaders() {
	glfwInit();
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
	glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);
	glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE);
